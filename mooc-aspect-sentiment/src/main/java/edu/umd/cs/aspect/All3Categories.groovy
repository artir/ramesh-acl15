...
class All3Categories{
	public static void main(String[] args)
	{
		...
		for(int i = 0; i <10; ++i)
		{
			All3Categories a = new All3Categories()
			a.pslmodel(i);
		}
	}
...
//		//fine2_seeded_1
//		
//		m.add rule : (fine2_seeded_1(P)) >> finetopic_2(P, data.getUniqueID(1)), weight : 5, squared:true
...
//		m.add rule : (fineseeded_12(P)) >> sentiment_1(P, data.getUniqueID(2)), weight : 10, squared:true
...
//		m.add rule : (fine2_seeded_10(P)) >> finetopic_1(P, data.getUniqueID(0)), weight : 5, squared:true
		...
		for (trainSet = 1 ; trainSet<=5;++trainSet)
		{
			...
			String filepath = ""
			filename = 'data'+java.io.File.separator+'4144folds_new'+java.io.File.separator+'fold'+dirToUse+java.io.File.separator;
			for (Predicate p : [fineseeded_1, fineseeded_2, fineseeded_3, fineseeded_4, fineseeded_5, fineseeded_6, fineseeded_7, fineseeded_8, fineseeded_9,
				...
				sentimentseeded3_1,sentimentseeded3_2,sentimentseeded3_3])
			{
			...
			for (Predicate p : [post, postfinecategory1, postsentimentcategory])
			{
				filepath = filename+p.getName().toString().toLowerCase()+".txt"
				println "\t\t\tREADING " + p.getName() +" from "+filepath+"...";
		...
		for (testSet=6;testSet<=10;++testSet){
			...
			String filepath = ""
			filename = 'data'+java.io.File.separator+'4144folds_new'+java.io.File.separator+'fold'+testfolder+java.io.File.separator;
			for (Predicate p : [fineseeded_1, fineseeded_2, fineseeded_3, fineseeded_4, fineseeded_5, fineseeded_6, fineseeded_7, fineseeded_8, fineseeded_9,
				fineseeded_10, fineseeded_11, fineseeded_12, fineseeded_13, fineseeded_14, fineseeded_15, fineseeded_16, fineseeded_17, fineseeded_18, fineseeded_19,
				sentimentseeded_1, sentimentseeded_2, sentimentseeded_3, sentimentseeded_4, sentimentseeded_5,
				sentimentseeded_6, senti_pos, senti_neg,sentimax,		fine1_negative_sum,fine2_negative_sum,fine1_neutral_sum,fine2_neutral_sum,fine2_availability_sum,fine1_content_sum,
sentimentseeded3_1,sentimentseeded3_2,sentimentseeded3_3])
			...
			String sentiment_1path = filename+"sentiment_1.txt"
			InserterUtils.loadDelimitedData(data.getInserter(sentiment_1, testLabelsPartition.get(cvSet)),
						sentiment_1path);
		...
		Set toClose = [fineseeded_1, fineseeded_2, fineseeded_3, fineseeded_4, fineseeded_5, fineseeded_6, fineseeded_7, 
			...
			fine2_availability_sum,fine1_content_sum,fine1_negative_sum,fine2_negative_sum,fine1_neutral_sum,fine2_neutral_sum,sentimentseeded3_1,sentimentseeded3_2,sentimentseeded3_3] as Set;
		...
		Database testLabelsDB = data.getDatabase(testLabelsPartition.get(cvSet), [finetopic_1,sentiment_1] as Set);
		...
		allGroundings = testDB.executeQuery(Queries.getQueryForAllAtoms(postsentimentcategory))
		println "groundings for all "+ allGroundings.size();
		for (int j = 0; j < allGroundings.size(); j++) {
			GroundTerm [] grounding = allGroundings.get(j)
		RandomVariableAtom atom2 = testDB.getAtom(sentiment_1, grounding);
		atom2.setValue(0.0);
		atom2.commitToDB();
		}
		...
		for (GroundAtom atom : Queries.getAllAtoms(testDB, sentiment_1)){
			file1.append( atom.toString() + "\t" + atom.getValue()+"\n");
		}
		...
		allGroundings = testDB.executeQuery(Queries.getQueryForAllAtoms(sentiment_1))
		System.out.println("printing no of sentiment atoms in test db " +allGroundings.size());
		
		testLabelsDB.close()
		def groundTruthDB = data.getDatabase(testLabelsPartition.get(cvSet), [finetopic_1, sentiment_1] as Set)
